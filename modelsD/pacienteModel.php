...
include_once "conexion.php";
	class paciente_model extends conexion_database {
		...
		private function editarPaciente(paciente_model $data){
			try {
				$query="UPDATE paciente set cedula=?,nombres=?,apellidos=?,fechaNacimiento=?,direccion=?,email=?,tlfno=?,sexo=? where id=?";
				$this->CNX->prepare($query)->execute(array($data->cedula,$data->nombres,$data->apellidos,$data->fechaNacimiento,$data->direccion,$data->correo,$data->telefono,$data->sexo,$data->id));
