...
class condicionmedica_model extends conexion_database {
	...
	private function modificarCondicionMedica(condicionmedica_model $data){
		try {
			$query="UPDATE condicion_medica set descripcion=?,observacion=?,status=? where id=?";
			$this->CNX->prepare($query)->execute(array($data->descripcion,$data->observacion,$data->status,$data->id));
