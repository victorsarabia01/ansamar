<?php
	...
	class consultorio_model extends conexion_database {
		...
		private function modificarConsultorio(consultorio_model $data){
			try {
				$query="UPDATE consultorio set descripcion=?,direccion=?,tlfno=?,sillas=?,status=? where id=?";
				$this->CNX->prepare($query)->execute(array($data->descripcion,$data->direccion,$data->telefono,$data->silla,$data->status,$data->id));
