<?php
	...
	class consultorio_model extends conexion_database {
		...
		private function registrarConsultorio(consultorio_model $data){
			try {
				$query="INSERT into consultorio (descripcion,direccion,tlfno,sillas,status) values (?,?,?,?,?)";
				$this->CNX->prepare($query)->execute(array($data->descripcion,$data->direccion,$data->tlfno,$data->silla,'1'));
		...
		private function listarConsultorios(){
			try {
				//$arreglo;
				$query="SELECT c.id,c.descripcion,c.direccion,c.tlfno,c.sillas,s.status as estado from consultorio as c INNER JOIN status as s where c.status=s.id";
				$smt = $this->CNX->prepare($query);
				$smt->execute();
		...
		private function modificarConsultorio(consultorio_model $data){
			try {
				$query="UPDATE consultorio set descripcion=?,direccion=?,tlfno=?,sillas=? where id=?";
				$this->CNX->prepare($query)->execute(array($data->descripcion,$data->direccion,$data->telefono,$data->silla,$data->id));
