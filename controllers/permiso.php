<?php
	...
	class permisoController extends controller {
		...
		public function __construct(){
			...
			$this->mode = new permiso_model();
			$this->alm = new permiso_model();
		...
		public function json(){
			if($this->accesoConsultar){
				$this->mode->Consultar("listarPermisos");
			}else{
		...
		public function editar(){
			if($this->accesoModificar){
				$this->alm = $this->mode->Consultar("cargarPermiso", $_REQUEST['id']);
				return $this->vista("permiso/modificar");
			}else{
				return $this->vista("error");
			}
		}
		...
		public function guardar(){
			if($this->accesoRegistrar){
				...
				if(!$this->alm->consulta==""){
					echo "2";
					/*echo "<script>
						alert('Ya existe un registro con este permiso');
						history.back();
					</script>";*/

				}else{
					$this->mode->Registrar("registrarPermiso", $this->alm);
					$this->bitacora->AnexarBitacora();
					echo "1";
					/*echo "<script>
						alert('Permiso Registrado');
						setTimeout( function() { window.location.href = 'index.php?c=permiso'; }, 1000 );
					</script>";*/
				}
			}else{
				return $this->vista("error");
		...
		public function modificar(){
			if($this->accesoModificar){
				...
				if($this->alm->consulta==$this->alm->consultaId){
					$this->mode->Editar("editarPermiso", $this->alm);
					$this->bitacora->AnexarBitacora();
					echo "1";
					/*echo "<script>
		...
		public function inhabilitar(){
			if($this->accesoEliminar){
				$this->mode->Eliminar("inhabilitarPermiso", $_REQUEST['id']);
				$this->bitacora->AnexarBitacora();
				echo "1";
				/*echo "<script>
		...
		public function habilitar(){
			if($this->accesoEliminar){
				$this->mode->Eliminar("habilitarPermiso", $_REQUEST['id']);
				$this->bitacora->AnexarBitacora();
				echo "1";
				/*echo "<script>
					alert('Modulo inhabilitado');
		...
		public function eliminar(){
			if($this->accesoEliminar){
				$this->mode->Eliminar("eliminarPermiso", $_REQUEST['id']);
				$this->bitacora->AnexarBitacora();
				echo "1";
				/*echo "<script>
					alert('Modulo inhabilitado');
		...
		public function verificarRegistroPermiso(){
		//$alm = new consultorio_model();
		if($this->accesoConsultar){
		$consultaBusqueda = $_POST['valorBusqueda'];
		foreach ($this->mode->Consultar("buscarRegistroPermiso", $consultaBusqueda) as $resultados) : 					
		$this->alm->id = $resultados->id;
    	endforeach;
    	if($this->alm->id != ""){
    		echo "
    			<div class='col-md-8'>
                <p style='color:Red;'> Permiso existe </p>
                </div>
    			";
    	}
    	}else{
		...
		public function cargarTablaPermisos(){
			if($this->accesoConsultar){
				$mensaje ="";
				$mensaje .='
					<div class="row">
					...
					<div class="row">
						<div class="col-md-12 text-center">
							<table class="table table-hover">
								...
								foreach ($this->mode->Consultar("listarPermiso") as $k){
									if($k->status==1){
										$estado='Activo';
									}else{
										$estado='Inactivo';
									...
									$mensaje .= '    
										<tr>
											...
											if($this->accesoModificar){
												$mensaje .='
												<td>
													<a href="index.php?c=permiso&a=editar&id='.$k->id.'" type="button" class="btn btn-outline-primary">
							...
							$(".eliminar").click(function(e){
								...
								swal({
									...
									if (isConfirm) {
										...
										window.location.href="index.php?c=permiso&a=inhabilitar&id="+id;
									} else {
								...
								<table class="table table-hover">
									...
									<tr>
										...
										if($this->accesoModificar){
											$mensaje .='
											<td>
												<a href="index.php?c=permiso&a=editar&id='.$this->alm->id.'" type="button" class="btn btn-outline-primary">
													<i class="bi bi-pencil-square"></i>
												</a>
											</td>';
										}
