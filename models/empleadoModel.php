<?php
	...
	class empleado_model extends conexion_database {
		...
		private function registrarEmpleado(empleado_model $data){
			try {
				$query="INSERT into empleado (cedula,nombres,apellidos,fechaNacimiento,email,codtlfno,tlfno,direccion,status,id_tipo_empleado) values (?,?,?,?,?,?,?,?,?,?)";
				$this->CNX->prepare($query)->execute(array($data->cedula,$data->nombres,$data->apellidos,$data->fechaNacimiento,$data->correo,$data->codtlfn,$data->telefono,$data->direccion,'1',$data->tipo_empleado));
		...
		private function listarEmpleado(){
			try {
				...
				$query="SELECT p.id,t.descripcion as cargo,p.cedula,CONCAT(p.apellidos ,' ', p.nombres) as full_name,p.codtlfno,p.tlfno,p.email,p.direccion ,es.status as estado FROM empleado AS p INNER JOIN tipo_empleado as t INNER JOIN status as es where p.id_tipo_empleado=t.id and p.status=es.id order by t.id";
				$smt = $this->CNX->prepare($query);
				$smt->execute();
		...
		private function modificarEmpleado(empleado_model $data){
			try {
				$query="UPDATE empleado set cedula=?,nombres=?,apellidos=?,fechaNacimiento=?,email=?,codtlfno=?,tlfno=?,direccion=?,id_tipo_empleado=? where id=?";
				$this->CNX->prepare($query)->execute(array($data->cedula,$data->nombres,$data->apellidos,$data->fechaNacimiento,$data->correo,$data->codtlfn,$data->telefono,$data->direccion,$data->tipo_empleado,$data->id));
