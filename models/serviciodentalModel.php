<?php
	...
	class serviciodental_model extends conexion_database {
		...
		private function modificarServicioDental(serviciodental_model $data){
			try {
				$query="UPDATE servicio_dental set nombre=?,descripcion=?,precio=? where id=?";
				$this->CNX->prepare($query)->execute(array($data->nombre,$data->descripcion,$data->precio,$data->id));
