<?php
	...
	class pagos_model extends conexion_database {
		...
		public function Consultar($metodo, $param1="", $param2=""){
			if($metodo=="buscarUltimoPagos"){ return self::buscarUltimoPagos(); }
			if($metodo=="listarPagos"){ return self::listarPagos(); }
			...
			if($metodo=="listarPagosReporte" && $param1!=""){ return self::listarPagosReporte($param1); }
			if($metodo=="cargarPagos" && $param1!=""){ return self::cargarPagos($param1); }
		}
		public function Registrar($metodo, $param1=[]){
			if($metodo=="registrarPagos" && !empty($param1)){ return self::registrarPagos($param1); }
		}
		...
		private function registrarPagos(pagos_model $data){
			try {
				...
				$this->CNX->prepare($query)->execute(array($data->id_evolucion, $data->fecha, $data->tipo, $data->tasa, $data->referencia, $data->monto, $data->equivalente, $data->leyenda, '1'));
			} catch (Exception $e) {
				die($e->getMessage());
			}
		}
		...
		private function buscarUltimoPagos(){
			try {
				$query="SELECT * FROM pagos WHERE status=1 ORDER BY pagos.id DESC";
				$smt = $this->CNX->prepare($query);
				$smt->execute();
				return $smt->fetch(PDO::FETCH_OBJ);
			} catch (Exception $e) {
		...
		private function listarPagosReporte($id){
			try {
				$query="SELECT *, pg.id as id_pago FROM paciente as p, cita as c, historia as h, evolucion as e, pagos as pg WHERE p.id=c.id_paciente and c.id=h.id_cita and h.id=e.id_historia and e.id=pg.id_evolucion and pg.status=1 and pg.id=".$id;
				$smt = $this->CNX->prepare($query);
				$smt->execute();
				return $smt->fetchAll(PDO::FETCH_OBJ);
			} catch (Exception $e) {
