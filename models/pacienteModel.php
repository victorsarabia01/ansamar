...
include_once "conexion.php";
	class paciente_model extends conexion_database {
		...
		public function Eliminar($metodo, $param1=""){
			if($metodo=="habilitarPaciente" && $param1!=""){ return self::habilitarPaciente($param1); }
			if($metodo=="inhabilitarPaciente" && $param1!=""){ return self::inhabilitarPaciente($param1); }
			if($metodo=="eliminarPaciente" && $param1!=""){ return self::eliminarPaciente($param1); }
		...
		private function editarPaciente(paciente_model $data){
			try {
				$query="UPDATE paciente set cedula=?,nombres=?,apellidos=?,fechaNacimiento=?,direccion=?,email=?,codtlfno=?,tlfno=?,sexo=? where id=?";
				$this->CNX->prepare($query)->execute(array($data->cedula,$data->nombres,$data->apellidos,$data->fechaNacimiento,$data->direccion,$data->correo,$data->codtlfn,$data->telefono,$data->sexo,$data->id));
		...
		private function listarPaciente(){
			try {
				//$arreglo;
				$query="SELECT p.id, p.cedula, CONCAT(p.nombres, ' ', p.apellidos) AS full_name,p.codtlfno,p.tlfno,p.email,s.status as estado from paciente as p INNER JOIN status as s where p.status=s.id";
				$smt = $this->CNX->prepare($query);
				$smt->execute();
		...
		private function inhabilitarPaciente($id){
			try {
		...
		private function habilitarPaciente($id){
			try {

				$query="UPDATE paciente set status='1' WHERE id=?";
				$smt = $this->CNX->prepare($query);
				$smt->execute(array($id));
				// return $smt->fetchAll(PDO::FETCH_OBJ);
		...
		private function eliminarCondicionMedicax($id){
			try {
