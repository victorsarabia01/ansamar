<?php
	...
	class historia_model extends conexion_database {
		...
		private function listarEnfermedadesCita($id_cita){
			try {
				$query="SELECT * from enfermedades as e, historia as h WHERE e.id = h.id_enfermedad and e.status=1 and h.id_cita=?";
				$smt = $this->CNX->prepare($query);
		...
		private function registrarHistoria(historia_model $data){
			...
			try {
				...
				$result = $this->CNX->prepare($query)->execute(array($data->id_cita, $data->id_enfermedad, $data->fecha, $data->diente, $data->cara, $data->presupuesto, "1"));
		...
		private function registrarHistoriaServicio(historia_model $data){
			try {
				...
				$result = $this->CNX->prepare($query)->execute(array($data->id_historia, $data->id_servicio, date("Y-m-d"), "1"));
				// $query="INSERT into historia (motivo,fecha,id_lesiones,id_tipo,observacion,id_tratamiento) VALUES (?,?,?,?,?,?)";
				// $this->CNX->prepare($query)->execute(array($data->motivo,$data->fecha,$data->id_lesiones,$data->dolor,$data->id_tipo,$data->observacion,$data->id_tratamiento));
				return $result;
			} catch (Exception $e) {
		...
		private function registrarEvolucionInsumo(historia_model $data){
			try {
				...
				$result = $this->CNX->prepare($query)->execute(array($data->id_evolucion, $data->id_insumo, $data->cantidad));
				// $query="INSERT into historia (motivo,fecha,id_lesiones,id_tipo,observacion,id_tratamiento) VALUES (?,?,?,?,?,?)";
				// $this->CNX->prepare($query)->execute(array($data->motivo,$data->fecha,$data->id_lesiones,$data->dolor,$data->id_tipo,$data->observacion,$data->id_tratamiento));
				return $result;
			} catch (Exception $e) {
		...
		private function actualizarHistoriaServicio(historia_model $data){
			try {
				...
				$smt = $this->CNX->prepare($query);
				$result = $smt->execute(array($data->evolucion, $data->observacion, $data->indicacion, $data->id_detalle));
				// return $smt->fetch();
				return $result;
			} catch (Exception $e) {
				die($e->getMessage());
		...
		private function actualizarAsignacionInsumo(historia_model $data){
			try {
				...
				$smt = $this->CNX->prepare($query);
				$result = $smt->execute(array($data->newUsada, $data->id_insumo));
				// return $smt->fetch();
				return $result;
			} catch (Exception $e) {
				die($e->getMessage());
		...
		private function actualizarEvolucionInsumo(historia_model $data){
			try {
				...
				$smt = $this->CNX->prepare($query);
				$result = $smt->execute(array($data->cantidad, $data->id_evInsumo));
				// return $smt->fetch();
				return $result;
			} catch (Exception $e) {
				die($e->getMessage());
